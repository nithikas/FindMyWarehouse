...
<div class="super_container" style="overflow: auto;">
	...
	<div class="home" style="margin-top:0px;">
		...
		$N1 = $_POST['zip_code'];
			...
			$N6 = $_POST['mobile_shelving'];
			if (empty($N6) == TRUE){
				$N6 = "false";
			}
			$N7 = $_POST['multi_tier_racking'];
			if (empty($N7) == TRUE){
				$N7 = "false";
			}
			$N8 = $_POST['mezzanine_flooring'];
			if (empty($N8) == TRUE){
				$N8 = "false";
			}
			$N9 = $_POST['price_lower_bound'];
			if (empty($N9) == TRUE){
				$N9 = "false";
			}
			$N10 = $_POST['price_higher_bound'];
			if (empty($N10) == TRUE){
				$N10 = "false";
			}
			$N11 = $_POST['space_lower_bound'];
			if (empty($N11) == TRUE){
				$N11 = "false";
			}
			$N12 = $_POST['space_higher_bound'];
			if (empty($N12) == TRUE){
				$N12 = "false";
			}
			$N13 = $_POST['entire_warehouse'];
			if (empty($N13) == TRUE){
				$N13 = "false";
			}
			$N14 = $_POST['year_built'];
			if (empty($N14) == TRUE){
				$N14 = "false";
			}
			$N15 = $_POST[lower_bound_owner_rating];
			if (empty($N15) == TRUE){
				$N15 = "false";
			}
			$N16 = $_POST[lower_bound_warehouse_rating];
			if (empty($N16) == TRUE){
				$N16 = "false";
			}
		?>
		...
		<div class = "main">
			...
			<?php
				...
				$newString = shell_exec("Rscript adv.R $N1 $N2 $N3 $N4 $N5 $N6 $N7 $N8 $N9 $N10 $N11 $N12 $N13 $N14 $N15 $N16");
				...
				if ($length > 0){
					...
					for ($x = 5; $x <= (strlen($newString) - 2); $x = $x + 13){
						...
						//To calculate space left, we need the contracts corresponding to the warehouse id
						$sql2 = "SELECT Start_Date_of_Lease, End_Date_of_Lease, Space_Rented FROM Contract WHERE Warehouse_ID = " . $substring;
						$result2 = $conn->query($sql2);
						$row2 = $result2->fetch_assoc();
						
						//Check if there is overlap in dates. If so, find the space remaining in the warehouse
